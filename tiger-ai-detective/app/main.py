    ...
    with st.form("new_case_form"):
        ...
        if submitted:
            ...
            try:
                ...
                new_case.ai_recommendations_generated_at = datetime.now()
                db.commit()
            ...
            with col4:
                if st.form_submit_button("🔄 Generate New AI Recommendations"):
                    ...
                    with st.spinner("Generating new AI recommendations..."):
                        ...
                        case.ai_recommendations_generated_at = datetime.now()
...
def display_case_details(case):
    ...
    try:
        ...
        if case.ai_recommendations:
            ...
            # Show when recommendations were last generated
            if case.ai_recommendations_generated_at:
                st.caption(f"🕒 Recommendations generated: {case.ai_recommendations_generated_at.strftime('%Y-%m-%d %H:%M')}")
            else:
                st.caption("🕒 Recommendations generated: Unknown time")
        ...
        if st.button("🔄 Generate AI Recommendations", key="generate_ai", help="Generate comprehensive AI research recommendations"):
            with st.spinner("🤖 Analyzing case and generating AI recommendations..."):
                ...
                if fresh_case:
                    ...
                    fresh_case.ai_recommendations_generated_at = datetime.now()
...
def research_assistant():
    """IBM Architecture & Integration Specialist - AI Research Assistant"""
    st.markdown("### 🏗️ IBM Architecture & Integration Specialist")
    st.markdown("""
    <div style="background-color: #f0f8ff; padding: 15px; border-radius: 8px; border-left: 4px solid #0066cc; margin-bottom: 20px;">
        <strong>🎯 Specialized Expertise:</strong> This AI assistant specializes in IBM enterprise architecture, 
        system integrations, and deployment strategies. Ask about architectural patterns, integration best practices, 
        performance optimization, and enterprise deployment scenarios.
    </div>
    """, unsafe_allow_html=True)
    
    # Check if Architecture Agent is available
    if not hasattr(st.session_state, 'architecture_agent') or not st.session_state.architecture_agent.is_initialized:
        st.warning("⚠️ Architecture Agent not initialized. Please check your API keys in the Admin page.")
        return
    
    # Add Architecture Agent status display
    status = st.session_state.architecture_agent.get_system_status()
    if status['initialized']:
        st.success(f"✅ Architecture Agent ready - {status['catalog_sources']} catalog sources loaded")
    else:
        st.warning("⚠️ Architecture Agent not fully initialized")
    
    # Specialized question categories
    st.markdown("#### 🎯 Quick Start Questions")
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("🏗️ Architecture Patterns", key="arch_patterns", help="IBM product integration patterns"):
            st.session_state.research_question = "What are the best architectural patterns for integrating WebSphere Application Server with IBM MQ and Db2 in a microservices environment?"
            st.rerun()
        
        if st.button("🔗 System Integration", key="sys_integration", help="How to connect IBM products"):
            st.session_state.research_question = "How should I architect the integration between IBM API Connect, WebSphere, and Db2 for a high-availability enterprise application?"
            st.rerun()
        
        if st.button("⚡ Performance Optimization", key="perf_opt", help="Architectural performance best practices"):
            st.session_state.research_question = "What are the architectural best practices for optimizing performance when integrating WebSphere, MQ, and Db2 in a distributed environment?"
            st.rerun()
    
    with col2:
        if st.button("☁️ Cloud Deployment", key="cloud_deploy", help="Cloud and hybrid deployment strategies"):
            st.session_state.research_question = "What are the recommended architectural patterns for deploying IBM WebSphere and Db2 in a hybrid cloud environment with OpenShift?"
            st.rerun()
        
        if st.button("🔒 Security Architecture", key="security_arch", help="Enterprise security patterns"):
            st.session_state.research_question = "How should I architect security for an IBM WebSphere, MQ, and Db2 integration in a regulated financial services environment?"
            st.rerun()
        
        if st.button("📊 Scalability Patterns", key="scalability", help="Scalable architecture design"):
    ...
    user_question = st.text_area(
        "Ask about IBM architecture, integration patterns, or deployment strategies:",
        height=120,
        placeholder="e.g., How should I architect WebSphere + MQ + Db2 integration? What's the best pattern for API Connect with WebSphere? How to design a scalable IBM product integration?",
        key="research_question",
        value=st.session_state.get("research_question", "")
    )
    
    if st.button("🏗️ Get Architecture Guidance", type="primary", key="research_button"):
        if user_question.strip():
            with st.spinner("🏗️ Analyzing IBM architecture patterns..."):
                if hasattr(st.session_state, 'architecture_agent') and st.session_state.architecture_agent.is_initialized:
                    response = st.session_state.architecture_agent.get_architecture_guidance(user_question)
                else:
                    response = "Architecture Agent not available. Please initialize the system first."
                ...
                # Display response
                st.markdown("### 🏗️ Architecture Guidance")
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🏗️ Architecture Specialist"):
    ...
    if st.session_state.current_case:
        ...
        with col2:
            if st.button("🏗️ Architecture Specialist", type="primary", use_container_width=True):
                st.session_state.show_research = True
                st.rerun()
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["📋 Create Case", "📊 View Cases", "🏗️ Architecture Specialist", "📜 History", "🔧 Admin"])
